...
End-to-end pipeline that:
  1. Generates a randomized-hook fun-fact script (30-60s, rotating tone/hook
     style for a distinct editorial voice) + title/description/tags/thumbnail
...
GROQ_MODEL = "openai/gpt-oss-20b"                      # llama-3.1-8b-instant was shut down Aug 16 2026; this is its replacement
FREESOUND_API_KEY = "FREESOUND_API_KEY_HERE"     # https://freesound.org/apiv2/apply (free, instant)
BACKGROUND_MUSIC_VOLUME = 0.12                         # 0.0-1.0, kept low so it never masks the voiceover
...
TARGET_DURATION_MIN_SECONDS = 30                        # randomized per-run video length range
TARGET_DURATION_MAX_SECONDS = 58                        # keep a couple seconds under 60s
MAX_SHORT_SECONDS = 60                                  # hard safety cap -- trims if the model overshoots
...
# Rotated per run so scripts don't all sound like the same templated bot.
# Deliberately excludes generic openers like "Did you know" / "Believe it or
# not" -- those are the exact filler hooks we're trying to get away from.
HOOK_STYLES = [
    "Open with a blunt, almost unbelievable claim stated as flat fact, then spend "
    "the rest of the script justifying it.",
    "Open by directly calling out the viewer ('You've definitely done X and had no idea...').",
    "Open with a sharp rhetorical question that is NOT a 'did you know' variant.",
    "Open with a wry, deadpan one-liner reacting to how strange the fact is, then explain it.",
    "Open mid-thought, like you're interrupting yourself to blurt out the wildest part first.",
    "Open by setting up a mini scenario or mental image, then reveal the twist.",
]

# Rotated alongside the hook style so the narration doesn't read the same every time.
EDITORIAL_TONES = [
    "wry and deadpan, like you find the fact mildly ridiculous",
    "excitable and nerdy, genuinely thrilled to be sharing this",
    "mildly sarcastic, with a light joke or two dropped in naturally",
    "warm and conversational, like explaining it to a curious friend",
    "dryly funny, treating an absurd fact completely matter-of-factly",
]


def _build_llm_prompt(target_seconds: int) -> str:
    ...
    `target_seconds` drives the requested word count (roughly 2.3-2.9
    words/sec of narrated speech) so the final video's length varies run to
    run instead of always landing around the same duration."""
    words_low = max(60, int(target_seconds * 2.3))
    words_high = int(target_seconds * 2.9)
    hook_style = random.choice(HOOK_STYLES)
    tone = random.choice(EDITORIAL_TONES)

    return (
        "You are a YouTube Shorts scriptwriter with a distinct, opinionated editorial "
        "voice -- not a generic trivia-bot. You write about random, surprising fun facts "
        "(science, space, history, animals, or psychology). Every script should sound "
        "like a specific host talking, not a templated fact card.\n\n"
        f"For THIS script, write the narration in a tone that is {tone}. "
        f"For the hook (the opening line), use this specific approach: {hook_style}\n\n"
        "Hard rules for the narration:\n"
        "- Do NOT start with 'Did you know', 'Believe it or not', 'Here's a fact', "
        "or any similarly generic filler opener.\n"
        "- Weave in at least one short editorial aside or reaction (a quick opinion, "
        "a joke, an 'honestly...' style comment) without breaking the flow of the fact.\n"
        f"- Target length: {words_low}-{words_high} words total (about {target_seconds} "
        "seconds narrated aloud). Use the space for specific numbers, context, or a twist "
        "-- never pad with filler to hit the count.\n\n"
        "Respond with ONLY a valid JSON object, no markdown fences, no commentary, "
        "matching exactly this schema:\n"
        "{\n"
        '  "fact": "The full narration script following the hook/tone/length rules above.",\n'
...
def _call_groq(prompt: str) -> str:
    ...
    openai/gpt-oss-20b (and 120b) are reasoning models: they spend part of
    their token budget on hidden "thinking" before writing the actual reply.
    reasoning_effort="low" keeps that budget small, reasoning_format="hidden"
    keeps the reasoning text out of the response body entirely, and
    max_completion_tokens (not the older max_tokens) is what Groq's docs use
    for these models -- together this avoids the reasoning eating the whole
    budget and leaving an empty `content` string."""
    ...
    if not content or not content.strip():
        raise RuntimeError("Groq returned an empty response (reasoning likely used the full token budget)")
    return content
...
def generate_fact_and_metadata(target_seconds: int, max_retries: int = 3) -> dict:
    """Generates the fun fact script + all video metadata in one LLM call,
    scaled to roughly `target_seconds` of narration. The hook/tone are
    randomized per call (see HOOK_STYLES / EDITORIAL_TONES) instead of
    forcing every script through the same 'Did you know?' template."""
    prompt = _build_llm_prompt(target_seconds)
    last_error = None
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            if not fact:
                raise ValueError("LLM returned an empty fact")
            data["fact"] = fact
            ...
            log.info("Generated fact (~%dw target %ss): %s", len(fact.split()), target_seconds, data["fact"])
            return data
        except Exception as e:
            last_error = e
...
# ==============================================================================
# 3b. BACKGROUND MUSIC (Freesound API, CC0-only)
# ==============================================================================
# Rotated per run for variety. Freesound content is user-uploaded, so results
# vary in quality -- these terms are chosen to bias toward calm instrumental
# beds rather than sound effects or vocal tracks.
RELAXING_MUSIC_QUERIES = [
    "calm ambient background music loop",
    "relaxing lofi background music",
    "soft piano ambient background",
    "peaceful acoustic background music",
    "gentle ambient pad relaxing",
]


def download_background_music(min_duration: float, output_path: str = None) -> str:
    """Searches Freesound for a calm instrumental track and downloads its
    preview-quality MP3 (no OAuth2 needed -- token auth covers previews).
    Restricted to license:"Creative Commons 0" (public domain) so the result
    is unambiguously free of copyright/attribution claims for YouTube."""
    if not FREESOUND_API_KEY or FREESOUND_API_KEY == "YOUR_FREESOUND_API_KEY_HERE":
        raise RuntimeError("FREESOUND_API_KEY is not set")

    output_path = output_path or str(OUTPUT_DIR / "background_music.mp3")
    ...
    params = {
        ...
        "filter": f'duration:[{int(min_duration)} TO 600] license:"Creative Commons 0"',
        "sort": "rating_desc",
        "page_size": 20,
    }
    ...
    try:
        ...
        if not results:
            # Loosen the duration filter -- we can loop a shorter track to fill the video
        ...
        if not results:
            raise ValueError(f"No CC0 Freesound results for query {query!r}")
        ...
        log.info("Downloaded background music: %r (%.0fs, CC0)", track["name"], track.get("duration", 0))
        return output_path
    except Exception as e:
        log.error("Background music download failed: %s", e)
        raise
...
def assemble_video(video_path: str, audio_path: str, words: list,
                    output_path: str = None, add_music: bool = True) -> str:
    ...
    # NOTE: moviepy 2.x removed the old `moviepy.editor` shim and renamed most
    # mutating methods to `with_*` / `*ed` (immutable-style) equivalents, e.g.
    # set_start -> with_start, subclip -> subclipped, resize -> resized,
    # crop -> cropped, set_audio -> with_audio, set_position -> with_position.
    from moviepy import (
        VideoFileClip, AudioFileClip, CompositeVideoClip, CompositeAudioClip,
        ImageClip, concatenate_videoclips, afx,
    )

    output_path = output_path or str(OUTPUT_DIR / "final_short.mp4")
    audio_clip = bg_clip = music_clip = mixed_audio = final = None
    caption_clips = []
    music_path = None
    ...
    try:
        ...
        mixed_audio = audio_clip  # default: voice only, in case music fetch/mix fails
        if add_music:
            try:
                music_path = download_background_music(min_duration=duration)
                music_clip = AudioFileClip(music_path)
                music_clip = music_clip.with_effects([afx.AudioLoop(duration=duration)])
                ...
                mixed_audio = CompositeAudioClip([audio_clip, music_clip]).with_duration(duration)
                log.info("Mixed in background music at %.0f%% volume", BACKGROUND_MUSIC_VOLUME * 100)
            except Exception as e:
                log.warning("Skipping background music (%s); continuing with voice-only audio", e)
                mixed_audio = audio_clip

        final = final.with_audio(mixed_audio).with_duration(duration)
        ...
        for clip in [*caption_clips, bg_clip, music_clip, mixed_audio, audio_clip, final]:
            try:
                if clip is not None:
                    clip.close()
            except Exception:
...
def main():
    ...
    try:
        target_seconds = random.randint(TARGET_DURATION_MIN_SECONDS, TARGET_DURATION_MAX_SECONDS)
        log.info("Step 1/6: Generating fact + metadata (target ~%ss)...", target_seconds)
        content = generate_fact_and_metadata(target_seconds)
        fact = content["fact"]
        title = content["title"]
